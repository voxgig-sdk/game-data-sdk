...
class GameDataMakeError
{
    public static function call(?GameDataContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new GameDataContext([], null);
        }
        $op = $ctx->op ?? new GameDataOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new GameDataResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof GameDataError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new GameDataError('', $msg, $ctx);
