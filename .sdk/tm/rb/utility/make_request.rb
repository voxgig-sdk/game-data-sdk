...
module GameDataUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = GameDataResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = GameDataResponse.new(fetched)
    else
