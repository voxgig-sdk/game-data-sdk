...
import (
	...
	omni "github.com/voxgig-sdk/game-data-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.GameDataError); is {
				out := map[string]any{
					"name":    "GameDataError",
...
func omniCtx(arg any, client *sdk.GameDataSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.GameDataSDK); is {
			client = live
