...
from gamedata_sdk.utility import register

# Load features
from gamedata_sdk.feature.base_feature import GameDataBaseFeature
from gamedata_sdk.features import _has_feature, _make_feature
...
class GameDataSDK:
    ...
    def get_utility(self):
        return GameDataUtility.copy(self._utility)
    ...
    def graphql(self, query, variables=None, ctrl=None):
        ...
        if isinstance(errors, list) and 0 < len(errors):
            ...
            res["ok"] = False
            res["err"] = Exception("GameDataSDK: graphql: " + str(msg))
            res["graphql"] = errors

        return res


    def GetGameById(self, data=None) -> "GetGameByIdEntity":
        """Entity factory: client.GetGameById().list() / client.GetGameById().load({"id": ...})."""
        from gamedata_sdk.entity.get_game_by_id_entity import GetGameByIdEntity
        return GetGameByIdEntity(self, data)


    def Popular(self, data=None) -> "PopularEntity":
        """Entity factory: client.Popular().list() / client.Popular().load({"id": ...})."""
        from gamedata_sdk.entity.popular_entity import PopularEntity
        return PopularEntity(self, data)



    @classmethod
    def test(cls, testopts=None, sdkopts=None) -> "GameDataSDK":
        if sdkopts is None:
            sdkopts = {}
