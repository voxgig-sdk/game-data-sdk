# GameData Python SDK Reference

Complete API reference for the GameData Python SDK.
...
#### `GameDataSDK.test(testopts=None, sdkopts=None)`
...
#### `GetGameById(data=None)`

Create a new `GetGameByIdEntity` instance. Pass `None` for no initial data.
...
```python
get_game_by_id = client.GetGameById()
```
...
Create a new `GetGameByIdEntity` instance with the same options.
...
| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `headerImage` | `str` | No | URL to game header image |
| `id` | `str` | No | Steam App ID |
| `name` | `str` | No | Game title |
| `popularity` | `int` | No | Popularity rank from SteamSpy |
| `releaseDate` | `str` | No | Game release date |
