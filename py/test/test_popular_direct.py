...
class TestPopularDirect:

    def test_should_direct_list_popular(self):
        setup = _popular_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-popular", "live" if setup["live"] else "unit")
        if _skip:
...
def _popular_direct_setup(mockres):
