...
class GameDataTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            GameDataTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = GameDataTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = GameDataTestRunner.getenv("GAME_DATA_TEST_LIVE")
        override = GameDataTestRunner.getenv("GAME_DATA_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = GameDataTestRunner.getenv(key)
        ...
        explain = GameDataTestRunner.getenv("GAME_DATA_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["GAME_DATA_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if GameDataTestRunner._test_control is not None:
            return GameDataTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                GameDataTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            GameDataTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = GameDataTestRunner.load_test_control()
...
def load_env_local():
    GameDataTestRunner.load_env_local()


def env_override(m):
    return GameDataTestRunner.env_override(m)


def entity_data(v):
    return GameDataTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return GameDataTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return GameDataTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return GameDataTestRunner.load_test_control()


def live_client_options():
    return GameDataTestRunner.live_client_options()


def live_delay_ms():
    return GameDataTestRunner.live_delay_ms()
