...
class TestGetGameByIdDirect:

    def test_should_direct_list_get_game_by_id(self):
        setup = _get_game_by_id_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-get_game_by_id", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
        if setup["live"]:
            for _live_key in ["get_game_by_id01"]:
...
def _get_game_by_id_direct_setup(mockres):
    ...
    if live:
        ...
        client = GameDataSDK(merged_opts)
        return {
            "client": client,
            "calls": calls,
            "live": True,
