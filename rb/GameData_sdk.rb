...
require_relative 'GameData_types'
...
class GameDataSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = GameDataHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = GameDataHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, GameDataFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = GameDataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = GameDataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = GameDataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = GameDataHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = GameDataError.new(
        "graphql_error", "GameDataSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  # Canonical facade: client.GetGameById.list / client.GetGameById.load({ "id" => ... })
  def GetGameById(data = nil)
    require_relative 'entity/get_game_by_id_entity'
    GetGameByIdEntity.new(self, data)
  end


  # Canonical facade: client.Popular.list / client.Popular.load({ "id" => ... })
  def Popular(data = nil)
    require_relative 'entity/popular_entity'
    PopularEntity.new(self, data)
  end
