...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = GameDataOmni.make_runner(TEST_JSON_FILE, GameDataSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : GameDataOmni.errify(err)
    end
