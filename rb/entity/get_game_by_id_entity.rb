...
class GetGameByIdEntity
  ...
  def make
    opts = @_entopts.dup
    GetGameByIdEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = GameDataHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [GetGameById, Hash] the current GetGameById data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = GameDataHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of GetGameById fields)
  def match_get
  ...
  #
  # @param reqmatch [GetGameByIdListMatch, Hash, nil] match filter (any subset of
  #   GetGameById fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<GetGameById>, Array] the matching GetGameById items; raises GameDataError on failure
