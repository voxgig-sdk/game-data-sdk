...
const TEST_SEED = { "entity": { "get_game_by_id": { "test01": { "id": "test01" } }, "popular": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+GameDataSDK\b/.test(code) ||
        /\bGameDataSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+GameDataSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/GameDataSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
