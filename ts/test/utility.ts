...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.GAMEDATA_TEST_LIVE ||
    'TRUE' === process.env.GAMEDATA_TEST_OVERRIDE
  ) {
  ...
  m.GAMEDATA_TEST_EXPLAIN = process.env.GAMEDATA_TEST_EXPLAIN || m.GAMEDATA_TEST_EXPLAIN
