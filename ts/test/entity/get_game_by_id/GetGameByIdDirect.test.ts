...
describe('GetGameByIdDirect', async () => {
  ...
  test('direct-list-get_game_by_id', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-get_game_by_id', setup.live)) return
    if (skipIfMissingIds(t, setup, ["get_game_by_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GAME_DATA_TEST_GET_GAME_BY_ID_ENTID']
