...
[MCP](https://modelcontextprotocol.io) server exposing the GameData SDK as
two agent tools — `game-data_list` and `game-data_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/game-data-mcp)
make build

# 2. Provide credentials via the environment
export GAME_DATA_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user game-data \
  -- /absolute/path/to/game-data-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./game-data-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// game-data_list: first page of records
{ "entity": "get_game_by_id" }
{ "entity": "get_game_by_id", "query": { } }

// game-data_load: one record by id
{ "entity": "get_game_by_id", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/game-data-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user game-data \
     -- "$PWD"/dist/*/game-data-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `game-data_list` and `game-data_load` tools now appear
   in new sessions. Ask the agent to *"list get_game_by_id using game-data"*
   and it calls `game-data_list` with `{"entity":"get_game_by_id"}`.
...
```sh
export GAME_DATA_APIKEY=sk_live_xxx            # API key
export GAME_DATA_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `game-data_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `game-data_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `GAME_DATA_APIKEY` | API key sent with every request. |
| `GAME_DATA_BASE` | Optional override of the API base URL. |

### Entities

The 2 entities valid as the `entity` argument:

get_game_by_id | popular

### Smoke test via HTTP (raw JSON-RPC)

```sh
./game-data-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"game-data_load","arguments":{"entity":"get_game_by_id","query":{"id":1}}}}'
```
