...
describe("GetGameByIdDirect", function()
  it("should direct-list-get_game_by_id", function()
    local setup = get_game_by_id_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-get_game_by_id", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"get_game_by_id01"}) do
