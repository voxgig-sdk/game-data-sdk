...
local PopularEntity = {}
PopularEntity.__index = PopularEntity


function PopularEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PopularEntity)
  self._name = "popular"
  self._client = client
...
function PopularEntity:get_name()
  return self._name
end


function PopularEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PopularEntity.new(self._client, opts)
end
...
function PopularEntity:mark_deleted()
  self._deleted = true
end


function PopularEntity:deleted()
  return true == self._deleted
end


function PopularEntity:data_set(args)
  if args ~= nil then
...
function PopularEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PopularEntity:match_set(args)
  if args ~= nil then
...
function PopularEntity:match_get()
...
function PopularEntity:stream(action, args, callopts)
...
---@param reqmatch PopularListMatch
---@param ctrl? table
---@return Popular[]
---@return string? err
function PopularEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function PopularEntity:_run_op(ctx, post_done)
  local utility = self._utility
