...
// GameData SDK GetGameById entity
...
class GetGameByIdEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new GetGameByIdEntity($this->_client, $opts);
    }

    /**
     * @param GetGameById|array $args GetGameById data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = GameDataHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return GetGameById|array The current GetGameById data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of GetGameById fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = GameDataHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of GetGameById fields).
     */
    ...
    /**
     * List GetGameById items matching the given filter.
     *
     * @param GetGameByIdListMatch|array|null $reqmatch Match filter (any subset
     *   of GetGameById fields) as an assoc-array; GetGameByIdListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return GetGameById[]|array A list of GetGameById items as assoc-arrays at
     *   the SDK boundary; throws GameDataError on failure (item-5 convention).
     */
