...
class FtCtrl extends GameDataControl
{
...
class FtClient
{
    ...
    private GameDataUtility $utility;

    public function __construct(GameDataUtility $utility, array $options)
    {
    ...
    public function get_utility(): GameDataUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (GameDataContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public GameDataContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (GameDataContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = GameDataConfig::make_feature($name);
    ...
    private static function buildUrl(GameDataSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof GameDataError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new GameDataResponse($fetched);
        }
    ...
    private function fail(GameDataContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (GameDataContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
