...
class GameDataNetsimFeature extends GameDataBaseFeature
{
    ...
    private function _simulate(GameDataContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(GameDataContext $ctx, array $applied): void
    {
        $client = $this->client;
