// Command GameData-cli is a CLI + REPL for the GameData SDK,
...
const entitiesHelp = "get_game_by_id popular"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      GAME_DATA_APIKEY (api key), GAME_DATA_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: GAME_DATA_APIKEY carries the API key and
	// GAME_DATA_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("GAME_DATA_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("GAME_DATA_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewGameDataSDK(opts)
