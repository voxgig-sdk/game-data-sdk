...
boru-driven command-line client **and** interactive REPL for the GameData
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/game-data-cli)
make build
...
./game-data-cli list get_game_by_id
./game-data-cli list popular

# 5. Override the API base URL for a single call
GAME_DATA_BASE=https://api.example.com ./game-data-cli list get_game_by_id

# 6. No arguments -> interactive REPL
./game-data-cli
game-data> list get_game_by_id
game-data> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/game-data-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/game-data-cli list get_game_by_id
   ```
...
Run with no arguments to open a REPL (prompt `game-data>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `get_game_by_id`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/game-data-cli`. |
...
(`./game-data-cli <expr>`) and interactively (the REPL), and expressions compose the
